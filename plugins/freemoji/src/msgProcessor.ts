		...
		if (
			...
			if (storage.hyperlink === true) {
				messageString = messageString.replace(
					emojiString[0], 
					`[${emojiString[1]}](https://cdn.discordapp.com/emojis/${emojiString[2]}.${ext}?size=${size}&name=${emojiString[1]})`
				);
			} else {
				messageString = messageString.replace(
					emojiString[0], 
					`https://cdn.discordapp.com/emojis/${emojiString[2]}.${ext}?size=${size}&name=${emojiString[1]}`
				);
			}
		}
	}
